...
- **Пропущено (неделя):** `1`, только если **дата планового платежа наступила**
  (`as_of >= next_planned_contribution_date`), недельный план > 0 и недельный факт <
  ожидания; иначе `0`. До срока платежа неделя **не** считается пропущенной (F4.11.1).
  Месяц/YTD (catch-up, информационно) = `ceil(разрыв / plan_weekly_rub)` если
  недельный план > 0, иначе `1` при разрыве > 0.
...
- **Статус (F4.11.1, привязан к дате планового платежа):**
  - `BEHIND` — срок платежа **наступил** (`as_of >= next_planned_contribution_date`)
    и основной разрыв > 0 (недельный, либо месячный если недельного плана нет);
  - `DUE_SOON` — срок ещё не наступил, но до него ≤ `DUE_SOON_WINDOW_DAYS` (7 дней)
    и есть предстоящий разрыв;
  - `ON_TRACK` — разрыва нет, либо срок далеко;
  - `NOT_STARTED` (до `plan_start_date`), `DISABLED` (`enabled=false`),
    `NOT_CONFIGURED` (плана нет).
  - Разрывы (`contribution_gap_weekly_rub` и др.) остаются **видимыми** как
    предстоящая/требуемая сумма даже до срока — это не «просрочка».
