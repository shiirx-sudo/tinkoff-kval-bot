...
def test_report_fields():
    ...
    for key in ("mode", "dry_run", "instruments_scanned", "included_by_profile",
                ...
                "unknown_income_count", "generated_profiles", "profile_set", "warnings"):
        assert key in rep
...
def test_profile_set_default_no_warning():
    res = B.build_universe(rules=RULES, mode="policy", profile_set="income",
                           dry_run=True, watchlist_fn=_fake_watchlist_fn)
    assert res.report["profile_set"] == "income"
    assert res.report["warnings"] == []


def test_profile_set_unsupported_warns():
    res = B.build_universe(rules=RULES, mode="policy", profile_set="bonds_only",
                           dry_run=True, watchlist_fn=_fake_watchlist_fn)
    assert res.report["profile_set"] == "bonds_only"
    assert res.report["warnings"]
    assert any("reserved/future" in w for w in res.report["warnings"])


# ─── header / source wording точные ──────────────────────────────────────────

def test_generated_header_source_wording():
    res = _build(mode="policy")
    text = B.render_universe_yaml(res, mode="policy", rules_path="test-rules")
    assert "# Auto-generated by build-income-universe." in text
    assert ("# Source: local rules + read-only T-Invest resolution/"
            "income-policy classification.") in text
    assert "# Enable mode: policy" in text


# ─── bond future filters не заявлены как работающие ──────────────────────────

def test_example_bond_filters_marked_future_not_enforced():
    data = yaml.safe_load(
        Path("config/income_universe_rules.example.yaml").read_text(encoding="utf-8"))
    bonds = data["filters"]["bonds"]
    # неэнфорсимые фильтры вынесены под future_filters, а не на верхний уровень
    assert "future_filters" in bonds
    for k in ("max_maturity_years", "min_coupon_frequency_per_year", "allow_floaters",
              "allow_fixed", "exclude_if_no_coupon_schedule", "exclude_if_unknown_currency"):
        assert k not in bonds, f"{k} must live under future_filters"
        assert k in bonds["future_filters"]
    # builder не читает future_filters (bond seeds только из candidate_tickers)
    seeds = B.gather_seeds(data)
    bond_seeds = [s for s in seeds if s.role == "bond_candidate"]
    assert bond_seeds == []   # candidate_tickers пуст
